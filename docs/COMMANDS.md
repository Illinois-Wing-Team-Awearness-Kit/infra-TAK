...
## 403 CSRF (same-origin) when updating CloudTAK or changing settings

If you see **Failed: 403: {"error":"CSRF validation failed (same-origin required)."}** when using the console behind Caddy (e.g. **Update** CloudTAK, **Apply** container log limits, or other POST/PUT/DELETE actions), the app is rejecting the request because the Origin or Referer host didn’t match the host the app thought it was serving on.

**Cause:** infra-TAK requires same-origin for state-changing API calls. When Caddy (or another proxy) sits in front, the app may see the backend host (e.g. `localhost:5001`) instead of the public host (e.g. `infratak.yourdomain.com`), so the check fails.

**Fix (current infra-TAK):** The app uses **X-Forwarded-Host** (and **X-Forwarded-Port** when non-standard) when present, so the same-origin check uses the host the browser sees. Ensure Caddy forwards the Host header. In your Caddy block for the console:
...
If your Caddyfile is generated by **Caddy SSL → Domains → Save & Reload Caddy**, the generated block may already forward the request; if you added a custom block, include the above so the console receives the public host. After changing Caddy, reload Caddy and refresh the console page; **Update** / **Apply** should then succeed.
...
git checkout dev -- \
  ...
  README.md \
  docs/COMMANDS.md \
  docs/RELEASE-v0.8.5-alpha.md \
  docs/RELEASE-v0.8.4-alpha.md \
  docs/RELEASE-v0.8.3-alpha.md \
  docs/RELEASE-v0.8.2-alpha.md \
...
python3 - <<'PY'
import re, sys
tag = "v0.8.5-alpha"  # change each release
want = tag.lstrip("v")
...
PY
git commit -m "v0.8.5-alpha"
git push origin main
git tag v0.8.5-alpha && git push origin v0.8.5-alpha
git checkout dev
```
