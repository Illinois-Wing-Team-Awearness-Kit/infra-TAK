...
- **v0.9.16 — Authentik worker CPU hotfix — on `dev`, pending test 2026-05-13** (see `docs/RELEASE-v0.9.16-alpha.md`). `_auto_remove_stale_docker_service_connections()` added to `_run_post_update()`. Deletes the "Local Docker" service connection that Authentik's upstream quickstart creates by default. The v0.9.2 CVE-2026-31431 hardening removed `/var/run/docker.sock` from the worker compose but left the service connection in Authentik's DB — worker's `outpost_service_connection_monitor` task retried the dead socket every 30s causing ~26% sustained CPU. Fix uses Authentik API `GET /api/v3/core/service_connections/docker/` + `DELETE` for each `local: true` connection. Idempotent, non-fatal.
